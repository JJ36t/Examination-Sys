...
class Student(db.Model):
    ...
    department = db.Column(db.String(50), default='علوم الحاسوب')  # 'علوم الحاسوب' أو 'نظم المعلومات'
...
def get_students_by_stage_semester():
    ...
    'department': student.department,  # إضافة معلومات القسم
    'grades': [],
...
def get_all_students_for_modification():
    ...
    for student in students:
        ...
        result.append({
            ...
            'student_id': student.student_id,
            'stage': student.stage,
            'department': student.department or 'علوم الحاسوب'
        })
...
def update_student():
    ...
    # التحقق من صحة البيانات
    if not all([student_id, user_id, name, username, student_id_number, stage, department]):
...
def create_default_admin():
    ...
    # التحقق من وجود سياق طلب قبل استخدام redirect
    try:
        from flask import has_request_context
        if has_request_context():
            return redirect(url_for('login'))
        ...
        # إذا لم يكن هناك سياق طلب، لا تستدعي redirect
        return None
...
def export_students():
    ...
    # تطبيق تصفية المرحلة إذا تم تحديدها
    if stage:
        query = query.filter(Student.stage == int(stage))
    ...
    for student, user in results:
        export_data.append({
            ...
            'name': user.name,
            'stage': student.stage,
            'department': student.department or 'غير محدد'
        })
    ...
    if export_format == 'excel':
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            df.columns = ['رقم الطالب', 'اسم المستخدم', 'الاسم', 'المرحلة', 'القسم']
            df.to_excel(writer, sheet_name='الطلاب', index=False)
            workbook = writer.book
            worksheet = writer.sheets['الطلاب']
            # تنسيق العرض
            for i, col in enumerate(df.columns):
                column_width = max(df[col].astype(str).str.len().max(), len(col) + 2) * 1.2
        ...
        output = io.StringIO()
        writer = csv.DictWriter(output, fieldnames=['student_id', 'username', 'name', 'stage', 'department'])
        writer.writeheader()
        writer.writerows(export_data)
        
        output_str = output.getvalue()
        response = make_response(output_str)
        response.headers["Content-Disposition"] = f"attachment; filename=students_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        response.headers["Content-type"] = "text/csv"
        return response
    
    else:
        flash('صيغة التصدير غير معتمدة', 'danger')
...
def import_students():
    ...
    # قراءة الملف والتعامل مع أنواع الملفات المختلفة
    try:
        if file.filename.endswith('.csv'):
            df = pd.read_csv(file)
            ...
            return jsonify({'error': 'صيغة الملف غير مدعومة. يجب استخدام CSV أو Excel'}), 400
        
        # تحويل أسماء الأعمدة إلى حروف صغيرة وإزالة المسافات
        df.columns = [str(col).lower().strip() for col in df.columns]
        
        # التحقق من وجود الأعمدة المطلوبة
        required_columns = ['name', 'username', 'stage']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            return jsonify({
                'error': f'الملف لا يحتوي على جميع الأعمدة المطلوبة. الأعمدة المفقودة: {", ".join(missing_columns)}'
            }), 400
        
        # تعيين القسم الافتراضي إذا لم يكن موجودًا في الملف
        default_department = request.form.get('default_department', 'علوم الحاسوب')
        ...
        # استيراد البيانات
        result = process_students_import(records, create_accounts, default_department)
        ...
        app.logger.error(f"خطأ أثناء استيراد الطلاب: {str(e)}")
        return jsonify({'error': f'حدث خطأ أثناء استيراد البيانات: {str(e)}'}), 500

def process_students_import(data, create_accounts=True, default_department='علوم الحاسوب'):
    """معالجة بيانات استيراد الطلاب"""
    success_count = 0
    error_count = 0
    errors = []
    ...
    for i, row in enumerate(data):
        try:
            # التحقق من وجود البيانات المطلوبة
            required_keys = ['name', 'username', 'stage']
            missing_keys = [key for key in required_keys if key not in row or pd.isna(row[key]) or str(row[key]).strip() == '']
            
            if missing_keys:
                app.logger.error(f"بيانات غير مكتملة في السجل {i+1}: {missing_keys}")
                errors.append({
                    'row': i + 1,
                    'error': f'بيانات غير مكتملة أو فارغة. الحقول المفقودة: {missing_keys}'
                })
            ...
            # التحقق من القسم وتعيين القيمة الافتراضية إذا كانت فارغة
            ...
            if department not in ['علوم الحاسوب', 'نظم المعلومات']:
                app.logger.error(f"القسم غير صالح في السجل {i+1}: {department}")
                errors.append({
                    'row': i + 1,
                    'error': f'القسم غير صالح. يجب أن يكون إما "علوم الحاسوب" أو "نظم المعلومات"'
                })
                error_count += 1
                continue
            ...
            app.logger.info(f"إضافة طالب جديد: ID={student_id}, المرحلة={stage}, القسم={department}")
            db.session.add(student)
            success_count += 1
...
# وظيفة تحديث هيكل قاعدة البيانات لإضافة عمود القسم
@app.route('/update_db_structure', methods=['GET'])
@login_required
def update_db_structure():
    if current_user.user_type != 'admin':
        return jsonify({'error': 'غير مصرح لك بتنفيذ هذه العملية'}), 403
    ...
    try:
        ...
        if 'department' not in column_names:
            # إضافة عمود القسم إلى جدول الطالب
            cursor.execute("ALTER TABLE student ADD COLUMN department TEXT DEFAULT 'علوم الحاسوب'")
            conn.commit()
            message = "تم إضافة عمود القسم بنجاح"
        else:
            message = "عمود القسم موجود بالفعل"
            
        conn.close()
        return jsonify({'success': True, 'message': message})
    except Exception as e:
        return jsonify({'error': f'حدث خطأ أثناء تحديث قاعدة البيانات: {str(e)}'}), 500

# وظيفة للتحقق من هيكل قاعدة البيانات وتحديثه إذا لزم الأمر
def check_and_update_db_structure():
    try:
        ...
        # التحقق من وجود جدول الطالب
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='student'")
        if cursor.fetchone():
            # التحقق مما إذا كان عمود department موجود
            cursor.execute("PRAGMA table_info(student)")
            columns = cursor.fetchall()
            column_names = [column[1] for column in columns]
            
            if 'department' not in column_names:
                print('جاري إضافة عمود department إلى جدول student...')
                # إضافة عمود القسم إلى جدول الطالب
                cursor.execute("ALTER TABLE student ADD COLUMN department TEXT DEFAULT 'علوم الحاسوب'")
                conn.commit()
                print('تم إضافة عمود department بنجاح!')
        conn.close()
        return True
    except Exception as e:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        create_default_admin()  # إنشاء حساب المسؤول الافتراضي
        check_and_update_db_structure()  # التحقق من هيكل قاعدة البيانات وتحديثه إذا لزم الأمر

    # تشغيل التطبيق
    if os.environ.get('RENDER'):
        # تشغيل على Render بدون SSL
